...
const ReceiveTrackingCard = ({ item, onCardClick }) => {
      ...
      <div className="dz-card style-1 overlay-shine">
        ...
        <div className="dz-info recommendation-card">
          ...
          <h4 className="dz-title">
            <Link to={`/Seguimiento-Detail/${item.id}`}>{item.title}</Link>
          </h4>
          <OverlayTrigger
          ...
          <div className="dz-button recommendation">
            <Link to={`/Seguimiento-Detail/${item.id}`} className="btn btn-primary btn-skew">
              <span>Leer más</span>
            </Link>
          </div>
        </div>
...
export default ReceiveTrackingCard;
