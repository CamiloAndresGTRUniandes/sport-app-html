...
const useReceiveTracking = () => {
  ...
  // Uso de useEffect para obtener seguimiento al cargar el componente.
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await axios.post(
          ...
          {
            "userId": user.id,
            "typeOfRecommendation": "f3510145-7a12-448a-b4ae-9cf65dfce907"
          },
          tokenPayload
        );
  ...
  const totalPages = Math.ceil(tracking.length / 3);

  // Retornar objetos y funciones útiles para el componente.
  return {
    tracking: tracking.slice(hover * 3, hover * 3 + 3),
    hover,
    setHover,
    totalPages,
    loading,
...
export default useReceiveTracking;
